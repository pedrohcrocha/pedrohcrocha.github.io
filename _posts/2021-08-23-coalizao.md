...
Seja $S \subseteq N$ os índices dos jogadores que fazem parte da coalizão.
...
Existe um $\alpha \in (0,1)$ tal que: 

$$p^s = \alpha p^{s^c}$$

Seja **m** o número de jogadores dentro da coalizão. Então, podemos obter:

$$sum_{i \in S} p_i^s + sum_{i \in S^c} p_i^{s^c} = 1$$
$$mp^s + (n-m)p^{s^c} = 1$$
$$\alphamp^{s^c} + (n-m)p^{s^c} = 1$$
